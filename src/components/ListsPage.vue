<template>
  <div class="lists-page">
    <div class="sticky top-0 z-20 bg-[#EFEEF7]/90 backdrop-blur-md px-5 pt-4 pb-3 border-b border-[#6D5FB1]/5">
      <div class="flex gap-2 bg-white/70 p-1.5 rounded-[18px]">
        ...
        >
          <span>{{ tab.icon }}</span>
          <span>{{ tab.name }}</span>
          <span v-if="getCount(tab.id) > 0" class="text-[10px] font-black px-1.5 py-0.5 rounded-full" :class="currentList === tab.id ? 'bg-white/30 text-white' : 'bg-[#EFEEF7] text-[#6D5FB1]'">{{ getCount(tab.id) }}</span>
        </button>
      </div>
    </div>
    ...
    <ListView
      ...
      :show-price="true"
      @add="(d: any) => listStore.addTodo(d.title, d.category, { price: d.price, budgetCategory: d.budgetCategory })"
