  ...
  <div class="app-container">
    ...
    <main class="main-content">
      ...
      <ListView v-if="currentTab === 'shop'" 
        ...
        @delete="tripStore.deleteShoppingItem" />
    ...
    <nav class="bottom-nav">
      <div class="nav-container">
        <button v-for="tab in tabs" :key="tab.id" @click="currentTab = tab.id" :class="{ active: currentTab === tab.id }">{{ tab.name }}</button>
      </div>
    </nav>
...
const tabs = [{id:'timeline', name:'行程'}, {id:'todo', name:'待辦'}, {id:'shop', name:'購物'}, {id:'budget', name:'預算'}]
...
const prepareAdd = () => {
  ...
  tempEvent.value = { id: Date.now().toString(), title: '', location: '', category: '景點', hour: 9, minute: 0, endHour: 10, endMinute: 0, budget: 0, dateIndex: tripStore.currentDayIndex, images: [] }
  isEditing.value = true
}

const openEdit = (event: TripEvent) => {
  editingIndex.value = tripStore.events.indexOf(event)
  tempEvent.value = JSON.parse(JSON.stringify(event))
  isEditing.value = true
}

const handleSave = (updatedEvent: TripEvent) => {
  if (editingIndex.value === -1) tripStore.addEvent(updatedEvent)
  else tripStore.events[editingIndex.value] = updatedEvent
...
.nav-container button { flex: 1; border: none; background: transparent; font-size: 13px; color: var(--rt-muted); font-weight: 700; }
