...
.header { 
  position: sticky; 
  top: 0; 
  background: rgba(239, 238, 247, 0.85); 
  backdrop-filter: blur(20px); 
  padding: 20px; 
  z-index: 90; 
  border-bottom: 1px solid rgba(109,95,177,0.1); 
}
...
.progress-fill { height: 100%; background: var(--rt-primary); transition: width 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
.nav-container { width: 100%; max-width: 420px; height: 72px; background: rgba(255,255,255,0.9); backdrop-filter: blur(20px); border-radius: 24px; display: flex; box-shadow: 0 15px 35px rgba(0,0,0,0.1); border: 1px solid rgba(255,255,255,0.5); }
.nav-container button { flex: 1; border: none; background: transparent; font-size: 13px; color: var(--rt-muted); font-weight: 700; cursor: pointer; transition: 0.2s; }
.nav-container button.active { color: var(--rt-primary); transform: translateY(-2px); }
</style>